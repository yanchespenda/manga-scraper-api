...
class LayoutBuilder {
  ...
  layoutDocument(docStructure, pdfDocument, styleDictionary, defaultStyle, background, header, footer, watermark, pageBreakBeforeFct) {
    function addPageBreaksIfNecessary(linearNodeList, pages) {
      if (!(0, _variableType.isFunction)(pageBreakBeforeFct)) {
        return false;
      }
      ...
      linearNodeList.forEach(node => {
        ...
        nodeInfo.stack = (0, _variableType.isArray)(node.stack);
  ...
  addBackground(background) {
    let backgroundGetter = (0, _variableType.isFunction)(background) ? background : () => background;
  ...
  addHeadersAndFooters(header, footer) {
    ...
    if ((0, _variableType.isFunction)(header)) {
      this.addDynamicRepeatable(header, headerSizeFct);
    } else if (header) {
      this.addStaticRepeatable(header, headerSizeFct);
    }

    if ((0, _variableType.isFunction)(footer)) {
  ...
  processTable(tableNode) {
    ...
    for (let i = 0, l = tableNode.table.body.length; i < l; i++) {
      ...
      if ((0, _variableType.isFunction)(rowHeights)) {
        height = rowHeights(i);
      } else if ((0, _variableType.isArray)(rowHeights)) {
        height = rowHeights[i];
      } else {
        height = rowHeights;
      }
  ...
  processLeaf(node) {
    ...
    if (line && line.inlines && (0, _variableType.isArray)(line.inlines)) {
...
function decorateNode(node) {
  ...
  if ((0, _variableType.isArray)(node.canvas)) {
