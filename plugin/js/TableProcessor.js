...
class TableProcessor {
  ...
  drawHorizontalLine(lineIndex, writer, overrideY) {
    ...
    if (lineWidth) {
      ...
      for (let i = 0, l = this.rowSpanData.length; i < l; i++) {
        ...
        if (borderColor == null) {
          borderColor = (0, _variableType.isFunction)(this.layout.hLineColor) ? this.layout.hLineColor(lineIndex, this.tableNode, i) : this.layout.hLineColor;
        }
  ...
  drawVerticalLine(x, y0, y1, vLineColIndex, writer, vLineRowIndex, beforeVLineColIndex) {
    ...
    if (borderColor == null) {
      borderColor = (0, _variableType.isFunction)(this.layout.vLineColor) ? this.layout.vLineColor(vLineColIndex, this.tableNode, vLineRowIndex) : this.layout.vLineColor;
    }
    ...
    for (let yi = skipOrphanePadding ? 1 : 0, yl = ys.length; yi < yl; yi++) {
      ...
      for (let i = 0, l = xs.length; i < l; i++) {
        ...
        if (i < l - 1) {
          ...
          if (!fillColor) {
            fillColor = (0, _variableType.isFunction)(this.layout.fillColor) ? this.layout.fillColor(rowIndex, this.tableNode, colIndex) : this.layout.fillColor;
          }

          if (!(0, _variableType.isNumber)(fillOpacity)) {
            fillOpacity = (0, _variableType.isFunction)(this.layout.fillOpacity) ? this.layout.fillOpacity(rowIndex, this.tableNode, colIndex) : this.layout.fillOpacity;
          }
