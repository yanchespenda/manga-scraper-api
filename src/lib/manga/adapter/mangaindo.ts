...
const MangaindoAdapter = {
	...
	supportsUrl(url) {
		return /^https?:\/\/(www\.)?mangaindo.web.id/.test(url);
	},
	...
	async getChapter(url) {
		...
		const $readerArea = dom('.entry-content').first();
		const $imageDom = $readerArea.find('img.aligncenter');
		...
		// const seriesId = dom('.nvs.nvsc').first();
...
export default MangaindoAdapter;
